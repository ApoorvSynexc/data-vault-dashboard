...
export default function BulkCsvScope({ sourceObjectNames, sourceObjectsLoading, onChange }: Props) {
  ...
  const activeCfg: CsvConfig = configByObj[activeObj] ?? blank();
  ...
  const totalIds = addedObjs.reduce((s, o) => s + (configByObj[o]?.parsedIds.length ?? 0), 0);
  ...
  return (
    <>
      <div className='rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden'>
          ...
          <div className='flex items-center gap-2 flex-wrap'>
            ...
            {addedObjs.length > 0 && (
              <span className='inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700'>
                {addedObjs.length} object{addedObjs.length !== 1 ? 's' : ''} · {totalIds} IDs
              </span>
            )}
          </div>
        </div>
          ...
          <div className='divide-y divide-gray-50' style={{ maxHeight: 280, overflowY: 'auto' }}>
            {sourceObjectNames
              ...
              .map((name) => {
                ...
                const idCount  = configByObj[name]?.parsedIds.length ?? 0;
                return (
                  <div key={name} className={`flex items-center gap-3 px-5 py-3 transition-colors ${isActive ? 'bg-blue-50' : isTicked ? 'bg-gray-50/60' : 'bg-white hover:bg-gray-50'}`}>
                    <input type='checkbox' checked={isTicked}
                      onChange={() => {
                        if (isTicked) {
                          ...
                          setActiveObj((prev) => prev === name ? (next[0] ?? '') : prev);
                        } else {
                          const next = [...addedObjs, name];
                          setAddedObjs(next);
                          setActiveObj(name);
                        }
                      }}
                      className='w-4 h-4 accent-blue-600 cursor-pointer rounded flex-shrink-0' />
                    <span className={`text-sm font-mono flex-1 min-w-0 truncate ${isActive ? 'font-semibold text-blue-700' : isTicked ? 'text-gray-800 font-semibold' : 'text-gray-600'}`}>{name}</span>
                    {isTicked && (
                      <button onClick={() => setActiveObj(name)}
                        className={`flex-shrink-0 text-[11px] font-semibold px-2.5 py-0.5 rounded-md border transition-colors ${isActive ? 'bg-blue-600 text-white border-blue-600' : 'border-blue-300 text-blue-600 hover:bg-blue-50'}`}>
        ...
        {addedObjs.length > 0 && activeObj && (
          <div className='border-t border-gray-200 p-5 space-y-4'>
            <div className='flex items-center gap-1 flex-wrap border-b border-gray-200'>
              {addedObjs.map((obj) => {
                const idCount = configByObj[obj]?.parsedIds.length ?? 0;
                const isActive = activeObj === obj;
                return (
                  <div key={obj} onClick={() => setActiveObj(obj)}
                    className={`flex items-center gap-1.5 px-3 py-2 cursor-pointer text-xs font-semibold border-b-2 transition-colors -mb-px ${isActive ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                    <span className='font-mono'>{obj}</span>
                    {idCount > 0 && (
                      <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${isActive ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>{idCount}</span>
                    )}
                  </div>
                );
              })}
            ...
            <input ref={fileInputRef} type='file' accept='.csv' className='hidden'
              onChange={(e) => { const f = e.target.files?.[0]; if (f) parseCsvFile(f, activeObj); e.target.value = ''; }} />
            ...
            <div className='border-2 border-dashed border-gray-300 rounded-xl px-6 py-8 text-center hover:border-blue-400 hover:bg-blue-50/30 transition-colors cursor-pointer'
              ...
              onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) parseCsvFile(f, activeObj); }}>
              ...
              {activeCfg.fileName
                ? <p className='text-sm font-semibold text-blue-600'>{activeCfg.fileName}</p>
                : <p className='text-sm font-semibold text-gray-700'>Drop a CSV for <span className='font-mono text-blue-600'>{activeObj}</span> here or click to browse</p>}
            ...
            <p className='text-xs font-semibold text-gray-700'>Or paste record IDs (one per line) for <span className='font-mono text-blue-600'>{activeObj}</span></p>
            <textarea value={activeCfg.text}
              onChange={(e) => {
                ...
                setConfigByObj((prev) => { const next = { ...prev, [activeObj]: { ...(prev[activeObj] ?? blank()), text, parsedIds: valid, errors, fileName: null } }; notifyChange(next, addedObjs); return next; });
              }}
              rows={5}
            ...
            {activeCfg.parsedIds.length > 0 && (
              <div className='flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-xs text-blue-800'>
                ...
                <span>
                  <strong>{activeCfg.parsedIds.length} valid ID{activeCfg.parsedIds.length !== 1 ? 's' : ''}</strong> loaded for <span className='font-mono'>{activeObj}</span>
                  {activeCfg.errors.length > 0 && <> · <span className='text-red-600 font-semibold cursor-pointer underline' onClick={() => setErrorOpen(true)}>{activeCfg.errors.length} error{activeCfg.errors.length !== 1 ? 's' : ''}</span></>}
                </span>
              </div>
            )}
            {activeCfg.errors.length > 0 && activeCfg.parsedIds.length === 0 && (
              <div className='flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-xs text-red-700 cursor-pointer' onClick={() => setErrorOpen(true)}>
                ...
                <span><strong>{activeCfg.errors.length} error{activeCfg.errors.length !== 1 ? 's' : ''}</strong> found — <span className='underline'>click to view</span></span>
              </div>
            )}
          </div>
        )}
