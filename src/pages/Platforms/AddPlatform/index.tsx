...
export default function AddPlatformModal({
  ...
  const { connectPlatform } = usePlatformService();
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-slate-900/35 p-4'>
      <div className='w-full max-w-md rounded-2xl border border-gray-200 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.2)]'>
        ...
        <div className='space-y-3 px-6 py-5'>
          {error && (
            <p className='rounded-lg bg-red-50 px-4 py-3 text-xs text-red-600'>{error}</p>
          )}
        ...
        <div className='flex justify-end gap-3 border-t border-gray-100 px-6 py-4'>
          ...
          <button
            type='button'
            disabled={!selected || connectPlatformMutation.isPending}
            onClick={() => selected && connectPlatformMutation.mutate(selected)}
          ...
          >
            ...
            {connectPlatformMutation.isPending ? 'Connecting...' : 'Connect'}
          </button>
        </div>
      </div>
    </div>
