...
// `fieldName` is the relationship field on this object pointing back to its parent
// (e.g. "AccountId") — named distinctly from the pre-existing `field` array key
// used elsewhere in the payload, so the two don't collide.
...
function buildPayloadTree(nodes: DepthChildNode[] = [], objects: BackupObject[]): PayloadChildNode[] {
  ...
  nodes.forEach((node) => {
    ...
    result.push({
      id: obj.uuid,
      name: obj.name,
      ...(node.field ? { fieldName: node.field } : {}),
          ...
          <button
            onClick={() => {
              const selectedObjectsData = Array.from(selectedObjects).map((uuid) => {
                ...
                // Only user-selected root objects carry children — same as onSelectionChange.
                // Auto-selected children must NOT carry children or they'll be treated as
                // parents on the next Back → remount cycle.
                const children = parentTreeMap.get(uuid);
