...
type PayloadChildNode = { id: string; name: string; fieldName?: string; cascadeDelete?: boolean; restrictedDelete?: boolean; children?: PayloadChildNode[] };
...
// `fieldName` (the relationship field back to the parent, e.g. "AccountId") is kept.
function stripRelationshipFlags(nodes: PayloadChildNode[]): { id: string; name: string; fieldName?: string; children?: unknown[] }[] {
  return nodes.map(({ id, name, fieldName, children }) => ({
    id,
    name,
    ...(fieldName ? { fieldName } : {}),
...
export default function FinalStep({
  ...
  return (
    <div className='h-full bg-gray-50 p-6 flex flex-col overflow-hidden'>
      ...
      <div className='flex items-start gap-2.5 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 mt-4'>
        ...
        <p className='text-xs text-amber-800 leading-relaxed'>
          <span className='font-semibold'>Note:</span> If any object has encrypted fields and you don't have the system permission <span className='font-semibold'>"View Encrypted Data"</span>, we will not be able to backup the value of those fields and their versions will also not be created.
        </p>
      </div>
              ...
              {isRealTime && (
                <>
                  ...
                  <ul className='text-sm text-gray-700 space-y-3 mb-4'>
                    ...
                    <li className='flex gap-2'><span className='mt-0.5 shrink-0 text-blue-600'>•</span><span>A Permission Set named <span className='font-semibold'>DataCraftRealTimeTriggerAccess</span> will be created in your Salesforce org and granted access to the DataCraft handler class and the triggers above.</span></li>
                    <li className='flex gap-2'><span className='mt-0.5 shrink-0 text-blue-600'>•</span><span><span className='font-semibold'>Action required:</span> Assign the <span className='font-semibold'>DataCraftRealTimeTriggerAccess</span> Permission Set to all users who create, update, or delete records on the selected objects.</span></li>
                  ...
                  <div className='bg-yellow-50 border-l-4 border-yellow-400 p-3 mb-4 rounded-r-lg'>
                    <p className='text-sm text-yellow-800'>By saving this configuration, you acknowledge that DataCraft will deploy <span className='font-semibold'>Apex Triggers</span> and a <span className='font-semibold'>Permission Set</span> to your connected Salesforce org as described above.</p>
                  </div>
                  <div>
